...
figure(2),imhist(uint8(b)), title('B histogram');;
...
figure(2), imshow(BWforeground),title('B&W segmented image');
...
figure(3), imshow(foregroundRGB),title('coloured foreground');
...
figure(4), imshow(uint8(blueness)), title('blueness channel'); 
%decide the threshold
figure(5),imhist(uint8(blueness)), title('blueness histogram');
threshold=input('Which threshold?'); % ask the user for a threshold
...
foregroundRGB=cat(3,uint8(foregroundR),uint8(foregroundG),uint8(foregroundB));
figure(6), imshow(foregroundRGB),title('coloured foreground using blueness');
